 ...
 * @throws
 */
const isArrayOfLabeledDescriptions = (array) => {
 ...
 */
const isArrayOfStringsAlphabetical = (array) => {
  if (array === null) return;
  module.exports.isArrayOfStrings(array, true);
};

const isArrayOfStrings = (array, alphabetical = false) => {
  if (array === null) return;
  if (typeof array === 'string') {
    array = JSON.parse(array);
  }
 ...
 *  }
 *
 * If 'effect' is an empty string, and save is non-zero, saveType not an empty string, then the save
 * must be made to avoid this damage
 */
const isDamageObject = (object) => {
